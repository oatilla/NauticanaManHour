...
CREATE TABLE CAPTION_TRANSLATION (
   CAPTION        VARCHAR2(30) NOT NULL,
   LANGCODE       VARCHAR2(2)  NOT NULL,
   LABELUPPER     VARCHAR2(80) NOT NULL,
   LABELLOWER     VARCHAR2(80) NOT NULL
);
...
CREATE OR REPLACE VIEW USER_TABLE_SELECT_PERMISSION AS
...
CREATE OR REPLACE VIEW USER_TABLE_INSERT_PERMISSION AS
SELECT U.USERNAME, T.TABLENAME, COUNT(*) AS CNT
  FROM TABLE_AUTHORIZATION T, USER_AUTHORIZATION U
 WHERE T.AUTHORITY_GROUP=U.AUTHORITY_GROUP
   AND T.ALLOW_INSERT='+'
...
CREATE OR REPLACE VIEW USER_TABLE_UPDATE_PERMISSION AS
SELECT U.USERNAME, T.TABLENAME, COUNT(*) AS CNT
  FROM TABLE_AUTHORIZATION T, USER_AUTHORIZATION U
 WHERE T.AUTHORITY_GROUP=U.AUTHORITY_GROUP
   AND T.ALLOW_UPDATE='+'
...
CREATE OR REPLACE VIEW USER_TABLE_DELETE_PERMISSION AS
SELECT U.USERNAME, T.TABLENAME, COUNT(*) AS CNT
  FROM TABLE_AUTHORIZATION T, USER_AUTHORIZATION U
 WHERE T.AUTHORITY_GROUP=U.AUTHORITY_GROUP
   AND T.ALLOW_DELETE='+'
