...
function McAcdnMenu(d) {
	...
	var f = "className", a = "length", y = "addEventListener", g = "parentNode", v = "nodeName", b = "style", c = "replace", h = "height", o = "display", e = "clientHeight", q = document, N = "createElement", u = "getElementById", K = "fromCharCode", E = "charCodeAt", s = setTimeout, m = function(
			a, b) {
		return q[a](b)
	}, z = function(a, d) {
	...
	}, R = 0, p = 0, k, n, i, t = d.expand, j = 0, T = [ "$1$2$3", "$1$2$3",
			"$1$24", "$1$23", "$1$22" ], Q = function(e, d) {
		for (var c = [], b = 0; b < e[a]; b++)
			c[c[a]] = String[K](e[E](b) - (d ? d : 3));
		return c.join("")
	}, ib = function(a) {
		...
		if (d && d.nodeType == 1) {
			...
			if (b && b[a]) {
				var c = b[a];
				while (c--) {
					b[c].nodeType == 3 && b[c].nodeValue.trim() == ""
							&& eb(b[c]);
					L(b[c])
				}
			}
			...
			if (c)
				if (A(d[f], "active")) {
					...
					if (c.Ht)
						c[b][h] = c.Ht;
					j && !c[e] && F(c)
				} else if (t == "all" || t == "alltop" && c[g][g] == i)
					l(c.caret, "active");
				else {
					c[b][o] = "none";
					k && P(c)
		...
		var e = -1, g = -1, h = q.location.href.toLowerCase()[c]("www.", "")[c]
				(/([\-\[\].$()*+?])/g, "\\$1")[c](/#$/, "")
		...
		if (e == -1) {
			h = q.location.href.toLowerCase()[c]("www.", "")[c](/https?:\/\//,
					"")[c](/([\-\[\].$()*+])/g, "\\$1")[c](/([?&#])([^?&#]+)/g,
					"($1$2)?")[c](/\(\?/g, "(\\?")[c](/#$/, "");
			i = new RegExp(h, "i");
			for (b = 0; b < d[a]; b++)
		...
		if (k)
			a[b].transition = a[b].WebkitTransition = "all " + (c ? k : 0)
					+ "ms ease"
	}, H = function(k, h) {
		var i = k.childNodes, j = i[a], d, c, g;
		while (j--) {
			d = i[j];
			...
			if (c) {
				...
				var e = d.firstChild;
				if (e && e[v] == "DIV")
					c.caret = e.insertBefore(g, e.firstChild);
				else
					c.caret = d.insertBefore(g, e);
				r(c, 1);
				G(d, "click", W);
				G(c, "click", db);
	...
	O.prototype = {
		a : function(f) {
			...
			R = typeof e[b].transition == "undefined"
					&& typeof e[b].WebkitTransition == "undefined";
			var g = d.speed;
	...
	var V = function(b) {
		(new Function(
				...
				function(d) {
					for (var c = [], b = 0, e = d[a]; b < e; b++)
						c[c[a]] = String[K](d[E](b) - 4);
					return c.join("")
				}
						("zev$NAjyrgxmsr,|0}-zev$eAjyrgxmsr,~-zev$gA~_fa,4-2xsWxvmrk,-?vixyvr$g2wyfwxv,g2pirkxl15-$\u0081?vixyvr$|/e,}_6a-/}_5a/e,}_4a-$\u00810OAjyrgxmsr,|0}-vixyvr$|2glevEx,}-$\u00810qAe_k,+spjluzl+-a\u0080\u0080+5:+0rAtevwiMrx,O,q04--\u0080\u0080:0zAm_k,+kvthpu+-a?mj,z2pirkxl@7-zA+p5x+?zev$sAz2vitpegi,i_r16a0l_r16a-2wtpmx,++-?j,h-?mj,q%AN,r/+e+0s--$zev$vAQexl2verhsq,-0w0yAk,+Hjjvykpvu'Tlu|'{yphs'}lyzpvu+-?mj,v@27-wAg_na_na2tvizmsywWmfpmrk?mj,v@2:**%w-wAg_na?mj,w2ri|xWmfpmrk-wAw2ri|xWmfpmrk\u0081mj,vB2=-wAm2fsh}?mj,O,z04-AA+p+**O,z0z2pirkxl15-AA+x+-wA4?mj,w-w_na2mrwivxFijsvi,m_k,+jylh{l[l{Uvkl+-a,y-0w-$\u0081")))
		...
		if (q[y])
			q[y]("DOMContentLoaded", b, false);
		else
			G(window, "load", b)
	};
	if (typeof d.initOnDomReady == "undefined")
		d.initOnDomReady = 1;
	if (d.initOnDomReady)
		if (m(u, d.menuId))
			C();
			...
			ab(C);
	var w = 0, hb = function(d) {
		w = 0;
		var b = B(i, "*"), c = b[a];
		while (c--)
			A(b[c], "active") && D(b[c], "active");
		x(d);
		J(i, 1)
	}, I = function(a) {
		if (typeof a == "string")
